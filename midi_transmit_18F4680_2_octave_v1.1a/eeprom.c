...
#ifdef EBAY_ENCODER
    __EEPROM_DATA(0x00,0x24,0x00,0x32,0xFF,0xFF,0xFF,0xFF);
#endif
    
#ifdef DIGIKEY_ENCODER
    __EEPROM_DATA(0x00,0x24,0x01,0x32,0xFF,0xFF,0xFF,0xFF);
#endif
...
void loadFromEEPROM(unsigned char slotNumber) {
    ...
    unsigned char to_load[save_slot_number_of_bytes];    
    
    //read from EEPROM
    for (unsigned int byte_count = 0; byte_count < save_slot_number_of_bytes; byte_count++){
    ...
    ARPEGGIO_ON                     = to_load[61];
    BPM                             = to_load[62];
    //calculate TMR0 here! (from BPM)
    ...
    skipFirstLast                   = to_load[70];
    LAST_NOTE_MODE_KEY_PRESS        = to_load[71];
    LAST_NOTE_MODE_KEY_RELEASE      = to_load[72];
    LAST_NOTE_MODE_TRANSPOSE        = to_load[73];
    LAST_NOTE_MODE_RESET            = to_load[74];
}
...
void saveToEEPROM(unsigned char slotNumber) {
    ...
    //64 bytes, counting all BOOLS as BYTES
    unsigned char to_save[save_slot_number_of_bytes];
    ...
    //arpeggiator settings
    to_save[61] = ARPEGGIO_ON;
    to_save[62] = BPM;
    to_save[63] = OCTAVES;
    to_save[64] = ARPEGGIO_TYPE;
    to_save[65] = NOTE_LENGTH_PULSES;
    to_save[66] = LATCH;
    to_save[67] = AUTO_LATCH;
    to_save[68] = UN_LATCH_MODE;
    to_save[69] = AUTO_RANDOMIZE;
    to_save[70] = skipFirstLast;
    to_save[71] = LAST_NOTE_MODE_KEY_PRESS;
    to_save[72] = LAST_NOTE_MODE_KEY_RELEASE;
    to_save[73] = LAST_NOTE_MODE_TRANSPOSE;
    to_save[74] = LAST_NOTE_MODE_RESET;

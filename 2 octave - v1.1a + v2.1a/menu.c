...
//this function prevents menu from blocking the rest of the code from running:
void un_block_code(void){
    ...
    //only read encoder turns
    if(which_functions_are_unblocked == encoder_only){
        processEncoderBuxton();     //rotary encoder pins are read in ISR, processed here
    }
    
    //read buttons, potentiometers and encoder turns
    if(which_functions_are_unblocked == buttons_and_potentiometers_too){
        digitalInputs();            //read digital inputs
        analogInputs();             //read analog inputs   !!! COMMENTED THIS OUT!!!   
        processEncoderBuxton();     //rotary encoder pins are read in ISR, processed here
    }

    // !!! LEFT OFF HERE !!!
    // comment this back in
    // when encoder is clicked, digits light up at TEMPO
    // should be BLANK!
    // also set which_functions_are_unblocked = encoder_only
    
    //PORTC &= 0xF0 // ---> ensure 7 segment display is blank    
    
    // !!! COMMENTED THIS OUT !!! --> "good enough"
    // !!! COMMENTED BACK IN !!! ---> display lights one digit at a time
    //TMR2ON = 1;                 //TIMER2 ON
}
...
void Function_A_1(void){
    ...
    while(encoder_click == 0) {
        ...
        //!!! copy this code to all  'A' menus !!!
        while(ON_OFF == 1){

            un_block_code();
            
            //print to LCD only once and fixed!
            if(print_row_2_once == 0){
                clear_row2();
                Lcd_Print_String(LCD_STRINGS[(modeMenu.selector % modeMenu.number) + modeMenu.address]);
                print_row_2_once = 1;
            }
        }        
...
void Function_A_2(void){
    ...
    while(encoder_click == 0) {
        ...
        while(ON_OFF == 1){
            ...
            if(print_row_2_once == 0){
                clear_row2();
                Lcd_Print_String(LCD_STRINGS[(triadMenu.selector % triadMenu.number) + triadMenu.address]);
                print_row_2_once = 1;
            }
        }
...
void Function_A_3(void){
    ...
    while(encoder_click == 0) {
        ...
        while(ON_OFF == 1){
            ...
            if(print_row_2_once == 0){
                clear_row2();
                Lcd_Print_String(LCD_STRINGS[(seventhMenu.selector % seventhMenu.number) + seventhMenu.address]);
                print_row_2_once = 1;
            }
        }        
...
void Function_A(void) {
    ...
    while(encoder_click == 0) {
        ...
        if (lcd_print_once == 0) {
            ...
            //KEYBOARD_MODE = keyboardMenu.selector; //comment out here
        }
        ...
        while(ON_OFF == 1){
            ...
            if(print_row_2_once == 0){
                clear_row2();
                Lcd_Print_String(LCD_STRINGS[(keyboardMenu.selector % keyboardMenu.number) + keyboardMenu.address]);
                print_row_2_once = 1;
            }
        }        
...
void Function_F(void) {
    ...
    programChange(channel, programChangeMenu.selector, programChangeMenu.selector); //make data1, data2 the same!
}
...
// turn arpeggio ON
// latch a chord
// display flickers a little

// click encoder
// 7 segment display flickers a lot

// un-latch chord
// display still flickers!

//turn arpeggio OFF
//display flicker gone!
...
void Function_H_1 (void) {
    ...
    // do un-latch if ARPEGGIATOR turned OFF
    if(arpeggiatorOnOffMenu.selector == 0){
